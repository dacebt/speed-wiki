...
as data on intents and events. The Worker runtime shell depends on the
behavior-only `@wikispeedrun/game` package; browser code never does.
...
behind the Worker later without touching anything else.

**4. One explicit Cloudflare runtime shell.** `pnpm dev` starts Vite with the
Cloudflare plugin. The same Worker origin serves the SPA, handles HTTP allocation and
native WebSockets, and allocates one named Room Durable Object per code. The object
validates and persists a versioned Room snapshot before sending a full sync. This path
covers protected Room creation, invited Membership joining, same-identity reconnection
across Durable Object eviction, deterministic one-live-Connection replacement,
disconnect grace and expiry, host removal of active or away Memberships, and
authenticated host Start through recoverable article preparation, a durable countdown,
one authoritative racing round, results, and host replay.
...
- A new client↔server interaction starts as a type in `shared/`, gets its game logic
  in `game/`, its transport in the Worker shell, and its rendering in one client feature.
...
- **The client fetches Wikipedia directly** (its REST API is CORS-open). The seam to
  proxy through the Worker exists for when caching or hop verification is wanted;
